...
# TODO: to check.
def _simulate_fn(ctx: Context, ntime: int, make_step_fn=Callable):
...
@eqx.filter_jit
def make_simulate_fn_fd(ctx: Context, ntime: int):
    return _simulate_fn(ctx, ntime, make_step_fn_fd)

@eqx.filter_jit
def make_simulate_fn(ctx: Context, ntime: int):
    return _simulate_fn(ctx, ntime, make_step_fn)