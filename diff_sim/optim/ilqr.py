...
def make_ilqr_step(qpos_init, step_fn, jac_fn, ctx):
    ...
    def linearize_dynamics_and_cost_ddp(X, U):
        ...
        def single_lin(x, u):
            ...
            # Always zero for rigid bodies : G(x).u 
    ...
    def backward_pass_ddp(f_x, f_u, f_xx, f_ux, c_x, c_u, c_xx, c_ux, c_uu, x_final):
        ...
        f_xx_rev = jnp.flip(f_xx, axis=0)
        f_ux_rev = jnp.flip(f_ux, axis=0)
        ...
        def bp_fn(carry, inp):
            V_x, V_xx = carry
            f_x_t, f_u_t, f_xx_t, f_ux_t, c_x_t, c_u_t, c_xx_t, c_ux_t, c_uu_t = inp

            Q_x = c_x_t + f_x_t.T @ V_x
            Q_u = c_u_t + f_u_t.T @ V_x
            Q_xx = c_xx_t + f_x_t.T @ V_xx @ f_x_t + jnp.einsum('i,ijk->jk', V_x, f_xx_t)
            Q_ux = c_ux_t + f_u_t.T @ V_xx @ f_x_t + jnp.einsum('i,ijk->jk', V_x, f_ux_t)
        ...
        (V_x_final, V_xx_final), (K_rev, k_rev) = jax.lax.scan(
            bp_fn, (V_x, V_xx),(f_x_rev, f_u_rev, f_xx_rev, f_ux_rev, c_x_rev, c_u_rev, c_xx_rev, c_ux_rev, c_uu_rev)
        )
