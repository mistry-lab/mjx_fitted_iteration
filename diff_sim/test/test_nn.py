...
class MLP(nnx.Module):
    def __init__(self, dims: list, rngs: nnx.Rngs):
        assert len(dims) == 4, "Expected dims=[2, 32, 32, 2]"

        self.linear1 = nnx.Linear(dims[0], dims[1], rngs=rngs)  # [2 → 32]
        self.dropout1 = nnx.Dropout(rate=0.1, rngs=rngs)
        self.bn1 = nnx.BatchNorm(dims[1], rngs=rngs)

        self.linear2 = nnx.Linear(dims[1], dims[2], rngs=rngs)  # [32 → 32]
        self.dropout2 = nnx.Dropout(rate=0.1, rngs=rngs)
        self.bn2 = nnx.BatchNorm(dims[2], rngs=rngs)

        self.linear3 = nnx.Linear(dims[2], dims[3], rngs=rngs)  # [32 → 2]

    def __call__(self, x: jax.Array):
        # x = nnx.gelu(self.dropout1(self.bn1(self.linear1(x))))
        # x = nnx.gelu(self.dropout2(self.bn2(self.linear2(x))))

        return self.linear3(nnx.relu(self.linear2(nnx.relu(self.dropout1(self.linear1(x))))))
...
if __name__ == '__main__':
    try:
        lr = 4e-3
        # Equinox Optimiser 
        net_eqx = ValueFunc([2, 64,64, 1], jax.random.PRNGKey(50))
        optim = optax.adamw(lr)
        opt_state = optim.init(eqx.filter(net_eqx, eqx.is_array))

        # NNX Optimiser
        net_nnx =ValueFuncNNX([2, 64,64,1], nnx.Rngs(0))
        optimizer_nnx = nnx.Optimizer(net_nnx, optax.adamw(0.1*lr))
        ...
        with jax.default_device(jax.devices()[0]):    
            ...
            f_sincos_2d = jax.vmap(sincos_2d, in_axes=(0))      
