...
model_path = os.path.join(os.path.dirname(__file__), '../xmls/ball_push.xml')
...
class Policy(Network):
    ...
    def __call__(self, x, t):
        ...
        x = jnp.concatenate([jnp.tanh(x[:2]) * 0.2, jnp.tanh(x[2:]) * 0.075], axis=0)
...
def policy(net: Network, mx: mjx.Model, dx: mjx.Data, policy_key: jnp.ndarray
    ...
    u = net(x, t)
    dx = dx.replace(qfrc_applied=dx.qfrc_applied.at[3:6].set(u))
